...
- **Voice Follow (development preview):** Use the waveform button in an active prompt. On Mac, you can also choose Teleprompter > Toggle Voice Follow or press `⌃⌥V`. Allow Microphone and Speech Recognition when Apple asks. Kyuva chooses only locales that Apple reports as available on device. If none is available for the detected script language, Kyuva leaves the prompt paused and shows an availability message instead of using cloud recognition.
...
Kyuva macOS version `1.0` is [available on the Mac App Store](https://apps.apple.com/ie/app/apple-store/id6804827338?pt=129302835&ct=github&mt=8). Its free listing was verified in all 27 European Union storefronts on 28 August 2026. The approved build removes the unfinished Mac remote and all Mac network access. iPhone and Apple Watch builds are currently source-build surfaces, so public mobile availability is not claimed.
