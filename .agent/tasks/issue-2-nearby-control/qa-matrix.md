...
|---|---|---|
| AC1 — standard local transfer | UNKNOWN physical | Actual iPhone simulator ShareLink → Save to Files → fileImporter round-trip passed with the exact committed 48-byte UTF-8 artifact. Physical Mac ↔ iPhone AirDrop/Files pass is still required. |
...
| AC3 — bounded remote protocol | PASS | Five commands only, 4 KiB frames, strict version/keys/sequence/replay checks, one-client server, and 31-test suite pass. |
...
| AC7 — deterministic proof | PASS local | 31 tests plus fresh macOS universal, iOS, and watchOS Release builds pass. The updated read-only verifier keeps only physical criteria blocked. |
